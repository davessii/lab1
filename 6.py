...
print(A)
A[-3:]
#Если вы хотите использовать From и To по умолчанию (все индексы по данной оси) а шаг отличный от 1, то вам необходимо использовать две пары двоеточий, чтобы интерпретатор смог идентифицировать единственный параметр как Step.
...
B = A.reshape(-1)
print(B.shape)
#При сложении массивов складываются значения каждого ряда. Делается это командой data+ones
data = np.array([1, 2])
ones = np.ones(2)
print(data+ones)
#Перевод миль в км
...
print (data.min())
print (data.sum())
print (data.mean()) #Ср. арифметическое отклонение
print (data.prod()) #Результат умножения всех элементов
